	...
	public static void main(String[] args) {
		...
		do {
			...
			case 1: // 아이돌 그룹 추가
				...
				break;
			case 2: // 아이돌 그룹 목록보기
				ArrayList<IdolGroupVO> idolGroupVOList = idolGroupService.getIdolGroupList();
			...
			case 4: // 아이돌 그룹 정보수정
				name = consoleUtil.getName(scanner);
				boolean overlapName = idolGroupService.getIsExistName(name);
				if(overlapName) { // 아이디가 존재하는 경우
		...
		case 5: // 아이돌 그룹 삭제
			name = consoleUtil.getName(scanner);
			overlapName = idolGroupService.getIsExistName(name);
			if(overlapName) { // 아이디가 존재하는 경우
